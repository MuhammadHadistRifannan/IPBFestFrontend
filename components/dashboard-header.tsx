...
export function DashboardHeader() {
  ...
  return (
    <header className="h-14 xl:h-16 border-b border-onyx-200/50 dark:border-onyx-800 bg-white/80 dark:bg-onyx-950 backdrop-blur-md flex items-center justify-between px-4 xl:px-6 sticky top-0 z-20">
      {/* Search Bar */}
      <div className="flex items-center w-full max-w-[200px] sm:max-w-sm relative">
        <Search className="absolute left-3 h-3.5 w-3.5 xl:h-4 xl:w-4 text-onyx-400 dark:text-onyx-500" />
        <Input
          type="search"
          placeholder="Cari analisis atau produk..."
          className="pl-9 xl:pl-10 h-8.5 xl:h-10 w-full rounded-lg xl:rounded-xl bg-onyx-50/50 dark:bg-onyx-900/50 border-onyx-200/60 dark:border-onyx-800 focus-visible:ring-chartreuse-500 text-xs xl:text-sm focus-visible:bg-white dark:focus-visible:bg-onyx-900 transition-all duration-200"
        />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <Button
                size="sm"
                className="h-8 xl:h-9 rounded-lg xl:rounded-xl bg-chartreuse-500 text-onyx-950 text-[11px] xl:text-xs font-bold hover:bg-chartreuse-600 shadow-md shadow-chartreuse-500/10 cursor-pointer flex items-center gap-1 xl:gap-1.5"
              >
                <Download className="h-3 w-3 xl:h-3.5 xl:w-3.5" />
                <span>Export</span>
              </Button>
            }
          />
          ...
          <DropdownMenuTrigger
            render={
              <Button
                variant="ghost"
                size="icon"
                className="h-8 w-8 xl:h-9 xl:w-9 rounded-lg xl:rounded-xl text-onyx-500 dark:text-onyx-400 hover:bg-onyx-50 dark:hover:bg-onyx-900/60 relative"
              >
                <Bell className="h-4.5 w-4.5 xl:h-5 xl:w-5" />
                <span className="absolute top-1.5 right-1.5 xl:top-2 xl:right-2 h-1.5 w-1.5 xl:h-2 xl:w-2 rounded-full bg-mauve-shadow-500" />
              </Button>
            }
          />
        ...
        <div className="h-5 xl:h-6 w-px bg-onyx-200 dark:bg-onyx-800 mx-0.5 xl:mx-1" />

        {/* User Profile Dropdown containing Theme Toggle */}
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <Button variant="ghost" className="relative h-8 w-8 xl:h-9 xl:w-9 rounded-full p-0 overflow-hidden border border-onyx-200 dark:border-onyx-800">
                <Avatar className="h-8 w-8 xl:h-9 xl:w-9 rounded-full">
                  ...
                  <AvatarFallback className="bg-onyx-100 dark:bg-onyx-800 text-onyx-700 dark:text-onyx-300 font-semibold text-xs xl:text-sm">
                    PP
                  </AvatarFallback>
                </Avatar>
              </Button>
